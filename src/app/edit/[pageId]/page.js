...
export default function EditPage() {
    ...
    const { getPageInfo , changePage } = usePages();
    ...
    useEffect(() => {
        async function fetchData() {
            const info = await getPageInfo(token['jwt-token'] , pageId);
            InputRef.current.value = info.name;
            DescriptionRef.current.value = info.description;
            setCardChoice(info.background_image);
            setColor1(info.background_color1);
            setColor2(info.background_color2);
        }
        fetchData();
    }, [pageId, router]);
    ...
    const handleSubmit = async () => {
        ...
        try{
            ...
            await changePage(token['jwt-token'] , pageId  , projectName ,description, css_type , bg_image ,color1 , color2);
